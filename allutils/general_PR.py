...
def flowPR(p_prime_nodes, ref_pr, stream, RS, current, iters=1000000, alpha=0.85, beta=0.001, gamma=0.9999, normalization=1.0, padding=0):
    ...
    for e in stream:
        ...
        if (i % 100 == 0 or i == len(stream)) and len(RS) == len(ordered_pr):
            if i == iters-1:
                print(sum(RS.values()))
            sorted_RS4 = np.array([RS[k] / sum(RS.values())
                                  for k in rank_order])
...
if __name__ == "__main__":
    ...
    G = allutils.graph_generator.weighted_DiGraph(
        n, p, seed=1.0, mode=mode, weights=weights)
    #nodes = G.nodes()

    # basic
    norm = sum(G.out_degree(weight='weight').values())
    sampling_edges = {e[:-1]: e[-1]['weight'] /
                      norm for e in G.edges_iter(data=True)}
    personalization = {k: v / norm for k,
                       v in G.out_degree(weight='weight').iteritems()}
    p_prime_nodes = {
        i: personalization[i]/G.out_degree(i, weight='weight') for i in G.nodes_iter()}
    pr = nx.pagerank(
    ...
    stream = [sampling_edges.keys()[i] for i in np.random.choice(
        range(len(sampling_edges)), size=iters, p=sampling_edges.values())]
    sorted_RS4, tau, spearman, pearson, error, epochs, top_k = flowPR_top(
        p_prime_nodes, pr, stream, iters=iters, beta=beta, gamma=gamma)
    ...
    p_prime_nodes = {
        i: personalization[i]/S.out_degree(i, weight='weight') for i in G.nodes_iter()}
    ...
    sorted_RS4, tau, spearman, pearson, error, x = flowPR(
        p_prime_nodes, sorted_pr, stream, iters=iters, beta=beta, gamma=gamma)
    print(tau[-1])
    ...
    personalization = {k: v / sum(personalization.values())
                       for k, v in personalization.iteritems()}
    ...
    plt.plot(x, error, 'b:')
    plt.legend(['tau: degree pers.', 'corr: degree pers.', 'error: degree pers.', 'tau: no pers.', 'corr: no pers.',
               'error: no pers.', 'tau: random pers.', 'corr: random pers.', 'error: random pers.'], loc=4)
